...
const errors={mode:'请选择有效的合作模式。',rules:'客户端版本不同，请刷新两个页面。',room:'房间不存在或已结束，请重新创建。',full:'房间已有两名玩家。',token:'重连凭据失效，请重新创建房间。',expired:'断线超过 30 秒，房间已结束。',already_connected:'这个席位已在另一个页面使用。请返回加入页，以另一名玩家加入。',room_limit:'房间已满，请稍后再试。',race:'请选择有效种族。',rate_limit:'操作过快，请稍候。',ended:'房间已结束。'};
...
function accept(m){
 ...
 $('phaseTitle').textContent=phase==='complete'?(c.mode==='conquest'?'共同征服完成':'三关合作试炼完成'):phase==='failed'?'防线失守':`${c.level} · 战前整备`;
 $('phaseSummary').textContent=phase==='complete'?'你们已完成本次合作战役。可结束房间，重新选择种族再玩。':phase==='failed'?'本关没有奖励。已购买部队与升级保留，可以返回商店后重新挑战。':`下一关：${(c.levelConfig.enemyRaces||[c.levelConfig.enemyRace]).map(r=>names[r]).join('、')}，每 ${c.levelConfig.waveInterval/24} 秒一排，持续出兵。${c.levelConfig.castle?'攻城积分':'突破数'}达到 ${c.levelConfig.target} 获胜，防线 ${c.levelConfig.lives} 点。`;
 renderMap(c,both&&m.status!=='ended');
 ...
 if(phase==='battle'){
  ...
  $('defense').textContent=`${c.levelConfig.castle?'攻城':'推进'} ${Math.floor(c.progress)} / ${c.progressMax} · 防线 ${c.defense} / ${c.defenseMax}`;
...
$('createRoom').onclick=()=>{if(!connecting)connect({type:'create',race:$('coopRace').value,mode:$('coopMode').value});};
