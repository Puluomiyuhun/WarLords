...
test('all nine races start with original four troops and have ladders plus two unique combat troops',()=>{
...
test('every homeland can conquer all 28 source regions through adjacent borders',()=>{
 for(const race of Object.keys(C.races)){let c=K.create(race,23);c.phase='map';let battles=0;while(c.phase!=='complete'){
  const targets=K.targets(c);assert.ok(targets.length);const r=targets[0],options=K.begin(c,r.id),b=new Battle(options);assert.equal(b.roster(0).length,4);assert.ok(Battle.restore(b.snapshot()));b.winner=0;b.players[0].kills=4;
  const money=c.army.gold,result=K.settle(c,b);assert.equal(c.army.gold,money+r.reward+20);assert.equal(K.settle(c,b),null);assert.equal(result.won,true);c=K.restore(c);if(c.phase==='shop')c.phase='map';battles++;assert.ok(battles<=25);
...
test('new ranged units launch distinct projectiles and heavy shots survive shields',()=>{
 for(const [race,id]of [['orc2',11],['woodelf',12],['human2',13],['elf',26],['troll',35],['human',27],['orc',21],['undead',23],['elf',24],['woodelf',30],['demon',47]]){
  ...
  // Keep the target stationary to measure the shot independently of sword movement.
