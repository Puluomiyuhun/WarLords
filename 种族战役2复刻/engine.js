...
class Battle{
 ...
 target(u,range){
  ...
  for(const v of this.units){if(v.dead||v.knockFrame||v.side===u.side||v.lane!==u.lane)continue;
 ...
 attack(u,hit){
  const v=this.units.find(x=>x.id===u.targetId);if(!v||v.dead||v.knockFrame||v.side===u.side||v.lane!==u.lane||Math.abs(v.x-u.x)>hit.range*2)return;
 ...
 fireMagic(u,kind){const v=this.units.find(v=>v.id===u.targetId);if(!v||v.dead||v.knockFrame||v.side===u.side||v.lane!==u.lane)return;
 ...
 fire(u){
  const v=this.units.find(x=>x.id===u.targetId);if(!v||v.dead||v.knockFrame||v.side===u.side||u.ammo<=0)return;u.ammo--;
  let range=v.x-u.x;
  if(v.moving){range=v.x+96*v.speed*v.dir-u.x;range=v.dir<0?Math.max(200,range):Math.min(-200,range);}
  let angle=Math.asin(range/2500)/2;if(!Number.isFinite(angle))angle=u.dir*Math.PI/4;
  this.projectiles.push({id:this.nextId++,owner:u.owner,side:u.side,lane:u.lane,x:u.x,y:-66,vx:50*Math.sin(angle),vy:-50*Math.cos(angle),targetId:v.id,firerType:u.type,age:0});
 ...
 move(u,ev){
  ...
  if(v&&(!this.def(u).ranged||!this.def(v).ranged))v.x+=amount*u.dir;
 }
 stepUnit(u){
  ...
  if(e.move)this.move(u,e);else u.moving=false;
  if(e.hit)this.attack(u,e.hit);
  if(e.impact&&typeof e.impact==='object')this.impact(u,e.impact);
  if(e.arrow){if(e.projectile)this.fireMagic(u,e.projectile);else this.fire(u);}
  u.frame=e.jump??u.frame+1;
  if(u.frame>(d.totalFrames||C.atlas['human-'+u.type].frames))u.frame=1;
  if(u.x*u.dir>1450){this.scores[u.side]++;u.exited=true;this.events.push({type:'score',side:u.side});}
 }
 ai(side){
 ...
 step(){
  ...
  for(const p of this.projectiles){
   ...
   p.age++;p.x+=p.vx;p.y+=p.vy;p.vy+=1;
   const v=this.units.find(x=>x.id===p.targetId);
   // Approximation of Flash hitTest: fixed bounds; kept isolated for later pixel-shape parity.
   if(v&&!v.dead&&!v.knockFrame&&v.side!==p.side&&p.y>-200&&p.y<-25&&Math.abs(p.x-v.x)<55){
    if(!v.shield){const d=C.units[p.firerType];let dmg=this.int(this.boost(50,p.owner??p.side,p.firerType,'attack'));if(d.bonusAgainst.includes(v.type))dmg*=1+d.bonus/100;this.hurt(v,dmg,{side:p.side,owner:p.owner??p.side});}p.done=true;
   }
 ...
 snapshot(){return JSON.parse(JSON.stringify({format:'warlords2-remake-battle',version:3,options:this.options,rng:this.rng,tick:this.tick,nextId:this.nextId,units:this.units,projectiles:this.projectiles,players:this.players,scores:this.scores,winner:this.winner,commands:this.commands,hits:this.hits}));}
 static restore(input){
  const s=structuredClone(input),o=s?.options,n=o?.mode==='coop'?3:2;
  if(!s||s.format!=='warlords2-remake-battle'||![1,2,3].includes(s.version)||!Number.isInteger(s.tick)||s.tick<0||s.tick>1000000||!Array.isArray(s.units)||s.units.length>3000||!Array.isArray(s.projectiles)||s.projectiles.length>10000||!Array.isArray(s.players)||s.players.length!==n||!Array.isArray(s.scores)||s.scores.length!==2||!Array.isArray(s.commands)||s.commands.length>100000)throw Error('存档格式不正确或版本不兼容');
  ...
  const b=new Battle(o);for(const k of ['rng','tick','nextId','units','projectiles','players','scores','winner','commands','hits'])b[k]=s[k];return b;
 }
