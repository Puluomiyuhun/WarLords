...
function spellNote(a,u){const names=Object.values(C.upgrades).filter(x=>x.kind==='magic'&&x.supported&&P.eligible(a.race,u.id,x.id)).map(x=>x.name);return names.length?'法术需另购，当前开放：'+names.join('、')+'。':'本兵种的法术尚未开放，目前仅有基础近战动作。';}
...
function render(){const a=army(),body=$('shopBody');body.replaceChildren();body.className='shop-body '+(tab==='army'||tab==='recruit'?'list-view':'detail-view');$('shopTitle').textContent=config.onAction?(config.ownerLabel||'玩家')+'的军队':'升级你的军队';$('shopGold').textContent='黄金: '+a.gold;$('shopContext').textContent=C.races[a.race].name+(tab==='recruit'?' · 添加新的单位':tab==='sell'?' · 卖出单位':'');$('shopArmyCount').textContent=`${a.roster.length} / 10`;$('shopEnabled').checked=!!config.enabled[owner];$('shopBack').textContent=tab==='army'?(config.campaign?'〈 征服地图':config.onAction?'〈 返回房间':'〈 主菜单'):'〈 返回';$('shopBack').disabled=busy;$('shopClose').disabled=busy;$('shopClose').textContent=config.onReady?'准备 〉':config.campaign?'征服地图 〉':'继续 〉';
 if(tab==='army'||tab==='recruit'){
  ...
  for(const id of ids){const u=C.units[id],row=el('article',undefined,'shop-unit');row.append(portrait(a.race,id),el('strong',u.name,'shop-unit-name'));
   if(tab==='army'){row.append(button('升级 / 信息',()=>show(id,'detail'),false,'shop-link shop-info-link'));if(!a.roster.slice(0,4).includes(id))row.append(button('卖出',()=>show(id,'sell'),false,'shop-link shop-sell-link'));}
 ...
 if(!owned){const upgrades=Object.values(C.upgrades).filter(x=>P.eligible(a.race,selected,x.id));const panel=el('div',undefined,'shop-available');panel.append(el('strong','可用升级'),el('p',upgrades.map(x=>x.name).join('、')||'无'));if(u.caster)panel.append(el('p',spellNote(a,u)));body.append(panel,button(a.roster.length>=10?'军队已满':`购买需要 ${u.price} 黄金`,()=>transact('buyUnit',selected,'已招募 '+u.name),a.roster.length>=10||a.gold<u.price,'shop-link shop-purchase'));return;}
 ...
 body.append(list,tip);if(!list.children.length)tip.append(el('p','暂无可购买的升级。'));else if(u.specialUpgrade>=0)tip.append(el('p','购买特殊能力后，双倍充能出兵可触发特殊动作。'));else if(u.caster)tip.append(el('p',spellNote(a,u)));
}
