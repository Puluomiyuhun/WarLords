...
function createHandler({origin,basePath}){
 return (req,res)=>{
  ...
  if(pathname==='/coop-health'){
   const body=JSON.stringify({app:'warlords2-local-coop',version:2,release:'0.6',basePath,public:!!origin});
  ...
  if(e.code==='EADDRINUSE'){
   try{const r=await fetch('http://127.0.0.1:'+port+config.basePath+'/coop-health',{signal:AbortSignal.timeout(2000)}),v=await r.json();
    if(v.app==='warlords2-local-coop'&&v.version===2&&v.basePath===config.basePath&&v.public===!!publicUrl){
     console.log('Co-op 0.6 already running: '+url);if(process.argv.includes('--open'))openBrowser(url);return;
    }
   }catch{}
  }
