...
function dismiss(a,type){if(!validateArmy(a)||a.roster.slice(0,4).includes(type)||!a.roster.includes(type))return false;a.roster=a.roster.filter(id=>id!==type);a.gold+=Math.round(C.units[type].price/2);return true;}
const api={BASE,baseRoster,MAX_ARMY,eligible,has,apply,validUpgrades,createArmy,validateArmy,buyUnit,buyUpgrade,dismiss};if(typeof module!=='undefined')module.exports=api;else g.Progression=api;
})(globalThis);
